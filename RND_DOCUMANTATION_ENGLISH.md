...
from the data.
- **`applyMissingMethods`** — apply methods for handling missing data.
- **`applyEncodingMethods`** — apply methods for encoding and scaling.
- **`generateCorrelationMatrix`** — build a correlation matrix.
- **`generateSelectedCorrelation`** — correlate selected features with others.
- **`loadCorrCheckboxes`** — load checkboxes for selecting features in the correlation matrix.
- **`showXYSelectors`** — show selectors for selecting X and Y.
- **`applyXYSelection`** — apply feature selection X and Y.
- **`applyTrainTestSplit`** — split the dataset into Train/Validation/Test.
- **`refreshTransformations`** — update the history of applied transformations.
- **`renderTaskButtons`** — render buttons for selecting the type of task.
- **`runRegressionModels`** — run regression models.
- **`runClassificationModels`** — run classification models.
- **`runClusteringModels`** — run clustering models.
- **`runNoTargetClusteringModels`** — run clustering without a target variable.
- **`runHybridPrediction`** — run mixed prediction (classification + regression).
- **`runMultiRegression`** — run MultiOutput regression.
- **`runMultiClassification`** — run MultiOutput classification.
- **`loadTargetFeaturesForHybrid`** — load target features for hybrid prediction.
- **`showHybridSelection`** — show selectors for selecting features in hybrid prediction.
- **`loadTaskButtons`** — load buttons for selecting tasks.
- **`openOverlay`** — open the overlay window.
- **`closeOverlay`** — close the overlay window.
- **`generateReport`** — generate a report with the results.

---

## Usage Logic
1. **Loading data** → `applyMissingMethods`, `applyEncodingMethods`.
2. **Analysis** → `generatePlot`, `generateCorrelationMatrix`, `generateSelectedCorrelation`.
3. **Feature selection** → `showXYSelectors`, `applyXYSelection`.
4. **Dataset splitting** → `applyTrainTestSplit`.
5. **Running models** → `runRegressionModels`, `runClassificationModels`, `runClusteringModels`, `runHybridPrediction`, `runMultiRegression`, `runMultiClassification`.
6. **Task selection** → `renderTaskButtons`, `loadTaskButtons`.
7. **Working with UI** → `openOverlay`, `closeOverlay`, `refreshTransformations`.
8. **Exporting results** → `generateReport`.

---
```mermaid
graph TD;

A[Load and process data] --> B[Analysis]
B --> C[X/Y feature selection]
...
E --> E2[Classification]
E --> E3[Clustering]
E --> E4[MultiOutput Regression]
E --> E5[MultiOutput Classification]
E --> E6[Hybrid Prediction]

E --> F[Update UI]
F --> F1[refreshTransformations]
F --> F2[renderTaskButtons]
F --> F3[loadTaskButtons]
F --> F4[openOverlay/closeOverlay]
