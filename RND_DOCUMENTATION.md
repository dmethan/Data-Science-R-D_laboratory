...
- **routers/** — FastAPI роутери:  
  ...
  - `manual_router.py` — маршрути для професійного режиму;
...
# Роутер ручного режиму (manual_router.py)

## Призначення
Роутер відповідає за роботу у **ручному режимі / AutoML**: завантаження датасету, його обробку, вибір ознак, запуск моделей (регресія, класифікація, кластеризація), а також експорт результатів.

---

## Логування та метадані
- **`def log_transformation`** — логування застосованих трансформацій.
- **`@router.get("/transformations")`** — отримати список трансформацій.
- **`@router.get("/save_transformations_log")`** — зберегти лог трансформацій.
- **`@router.get("/export_metadata_json")`** — експорт метаданих у JSON.

---

## Завантаження та попередня обробка
- **`@router.post("/upload")`** — завантаження CSV‑файлу.
- **`@router.get("/get_columns")`** — отримати список колонок датасету.
- **`@router.get("/analysis/plot")`** — побудова графіків для аналізу даних.
- **`@router.post("/apply_missing")`** — застосування методів обробки пропусків.
- **`@router.post("/apply_encoding")`** — застосування методів кодування та масштабування.
- **`@router.get("/correlation_matrix")`** — побудова кореляційної матриці.
- **`@router.post("/correlation_selected")`** — кореляції вибраних ознак з іншими.

---

## Вибір ознак та розділення датасету
- **`@router.post("/apply_xy")`** — вибір ознак X та Y.
- **`@router.post("/train_test_split")`** — розділення на Train / Validation / Test.
- **`@router.get("/download_split/{part}")`** — завантаження окремої частини датасету.
- **`@router.get("/download_split_zip")`** — завантаження Train/Val/Test у ZIP.
- **`@router.get("/download_full_dataset")`** — завантаження всього обробленого датасету.
- **`@router.get("/download_xy_zip")`** — завантаження X та Y у ZIP.
- **`@router.get("/get_y_info")`** — інформація про Y ознаки.

---

## Налаштування задачі
- **`@router.post("/start_task")`** — вибір типу задачі (класифікація, регресія, кластеризація).
- **`def to_native(obj)`** — перетворення numpy типів у стандартні Python.

---

## Моделі та функції
- **`def run_regression_models_random`** — запуск регресії з RandomizedSearch.
- **`def run_classification_models_random`** — запуск класифікації з RandomizedSearch.
- **`def run_clustering_models`** — кластеризація з цільовою змінною.
- **`def run_no_target_clustering`** — кластеризація без цільової змінної.
- **`def run_multi_regression`** — мульти‑регресія.
- **`def run_multi_classification`** — мульти‑класифікація.

---

## Ендпоінти для запуску моделей
- **`@router.post("/run_regression_random")`** — запуск регресійних моделей.
- **`@router.post("/run_classification_random")`** — запуск класифікаційних моделей.
- **`@router.post("/run_clustering")`** — запуск кластеризаційних моделей.
- **`@router.post("/run_no_target_clustering")`** — запуск кластеризації без цільової змінної.
- **`@router.post("/run_multi_regression")`** — запуск мульти‑регресії.
- **`@router.post("/run_multi_classification")`** — запуск мульти‑класифікації.
- **`@router.post("/run_multi_regression")`** — запускає MultiOutput регресійні моделі (RandomForest, GradientBoosting).  
  Використовується для задач, де потрібно прогнозувати кілька числових цільових ознак одночасно.
...
- **`@router.post("/run_multi_classification")`**  
  ...
  Використовується для задач, де потрібно класифікувати кілька категоріальних ознак одночасно.


- **`@router.post("/run_hybrid_prediction")`**  
  Запускає змішаний прогноз: одночасно класифікацію та регресію для різних цільових ознак.  
  Виконує RandomizedSearchCV для підбору параметрів, формує топ‑3 моделей за метриками, додає графіки (Confusion Matrix, ROC, PR, Scatter, Residuals, Q‑Q).


- **`@router.get("/get_target_features_for_hybrid")`**  
  Отримує список цільових ознак для змішаного прогнозу.  
  Повертає назви колонок, які були вибрані як Y (цільові змінні).

---

## Пояснення
- Ендпоінти групуються за призначенням: **логування**, **завантаження/обробка**, **вибір ознак**, **моделі**.
- Кожен ендпоінт має чітке призначення: від завантаження даних до запуску конкретних моделей.
- Функції `run_*` — це внутрішні реалізації алгоритмів, а `@router.post/get` — API‑ендпоінти для взаємодії з користувачем.

```mermaid
graph TD;

    A[Завантаження CSV] --> B[Попередня обробка даних]
    B --> C[Вибір ознак X та Y]
    ...
    E --> E2[Класифікація]
    E --> E3[Кластеризація]
    E --> E4[MultiOutput Регресія]
    E --> E5[MultiOutput Класифікація]
    E --> E6[Змішаний прогноз]

    E1 --> F[Експорт результатів]
    E2 --> F
    E3 --> F
    E4 --> F
    E5 --> F
    ...
    F --> G[Завантаження у CSV/ZIP/JSON]
...
## Призначення
Файл містить клієнтські JavaScript‑функції для роботи у **ручному режимі / AutoML**.  
Всі функції підключені до `window.*`, щоб їх можна було викликати напряму з HTML.

---

## Список функцій

- **`generatePlot`** — побудова графіків для аналізу даних.
- **`applyMissingMethods`** — застосування методів обробки пропусків.
- **`applyEncodingMethods`** — застосування методів кодування та масштабування.
- **`generateCorrelationMatrix`** — побудова кореляційної матриці.
- **`generateSelectedCorrelation`** — кореляція вибраних ознак з іншими.
- **`loadCorrCheckboxes`** — завантаження чекбоксів для вибору ознак у кореляційній матриці.
- **`showXYSelectors`** — показ селекторів для вибору X та Y.
- **`applyXYSelection`** — застосування вибору ознак X та Y.
- **`applyTrainTestSplit`** — розділення датасету на Train/Validation/Test.
- **`refreshTransformations`** — оновлення історії застосованих трансформацій.
- **`renderTaskButtons`** — рендеринг кнопок для вибору типу задачі.
- **`runRegressionModels`** — запуск регресійних моделей.
- **`runClassificationModels`** — запуск класифікаційних моделей.
- **`runClusteringModels`** — запуск кластеризаційних моделей.
- **`runNoTargetClusteringModels`** — запуск кластеризації без цільової змінної.
- **`runHybridPrediction`** — запуск змішаного прогнозу (класифікація + регресія).
- **`runMultiRegression`** — запуск MultiOutput регресії.
- **`runMultiClassification`** — запуск MultiOutput класифікації.
- **`loadTargetFeaturesForHybrid`** — завантаження цільових ознак для гібридного прогнозу.
- **`showHybridSelection`** — показ селекторів для вибору ознак у гібридному прогнозі.
- **`loadTaskButtons`** — завантаження кнопок для вибору задачі.
- **`openOverlay`** — відкриття overlay‑вікна.
- **`closeOverlay`** — закриття overlay‑вікна.
- **`generateReport`** — генерація звіту з результатами.

---

## Логіка використання
1. **Завантаження даних** → `applyMissingMethods`, `applyEncodingMethods`.  
2. **Аналіз** → `generatePlot`, `generateCorrelationMatrix`, `generateSelectedCorrelation`.  
3. **Вибір ознак** → `showXYSelectors`, `applyXYSelection`.  
4. **Розділення датасету** → `applyTrainTestSplit`.  
5. **Запуск моделей** → `runRegressionModels`, `runClassificationModels`, `runClusteringModels`, `runHybridPrediction`, `runMultiRegression`, `runMultiClassification`.  
6. **Вибір задачі** → `renderTaskButtons`, `loadTaskButtons`.  
7. **Робота з UI** → `openOverlay`, `closeOverlay`, `refreshTransformations`.  
8. **Експорт результатів** → `generateReport`.  

---
```mermaid
graph TD;

    A[Завантаження та обробка даних] --> B[Аналіз]
    B --> C[Вибір ознак X/Y]
