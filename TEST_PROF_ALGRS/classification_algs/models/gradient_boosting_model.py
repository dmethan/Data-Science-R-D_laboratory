...
def train(df, target, params: dict = None):
    """
    Тренування моделі GradientBoostingClassifier.
    Параметри приймаються через словник params.
    """

    logger.info("=== Запуск функції train (GradientBoostingClassifier) ===")
    ...
    logger.info("Модель GradientBoostingClassifier створена")
    ...
    save_model(model_package, "gradient_boosting_model")
    logger.info("Модель збережена у файл gradient_boosting_model.pkl")

    logger.info("=== Кінець функції train (GradientBoostingClassifier) ===")
