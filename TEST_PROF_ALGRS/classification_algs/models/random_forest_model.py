...
def train(df, target, params: dict = None):
    """
    Тренування моделі RandomForestClassifier.
    Параметри приймаються через словник params.
    """

    logger.info("=== Запуск функції train (RandomForestClassifier) ===")
    ...
    logger.info("Модель RandomForestClassifier створена")
    ...
    save_model(model_package, "random_forest_model")
    logger.info("Модель збережена у файл random_forest_model.pkl")

    logger.info("=== Кінець функції train (RandomForestClassifier) ===")
