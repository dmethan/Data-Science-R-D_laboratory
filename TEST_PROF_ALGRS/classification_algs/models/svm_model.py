...
def train(df, target, params: dict = None):
    ...
    logger.info("=== Запуск функції train (SVC) ===")
    ...
    logger.info("Модель SVC створена")
    ...
    save_model(model_package, "svm_model")
    logger.info("Модель збережена у файл svm_model.pkl")

    logger.info("=== Кінець функції train (SVC) ===")
