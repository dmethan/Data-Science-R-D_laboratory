...
def train(df, target: str = None, params: dict = None):
    """
    Тренування моделі MeanShift.
    Параметри приймаються через словник params.
    """

    logger.info("=== Запуск функції train (MeanShift) ===")
    ...
    logger.debug(f"model_params (до bandwidth): {model_params}")

    # Якщо bandwidth не задано — оцінюємо автоматично
    if model_params["bandwidth"] is None:
        data_for_bw = df.drop(columns=[target]) if target else df
        model_params["bandwidth"] = estimate_bandwidth(data_for_bw, quantile=0.2)
        logger.info(f"Автоматично оцінено bandwidth: {model_params['bandwidth']}")

    # Створюємо модель
    model = MeanShift(**model_params)
    logger.info("Модель MeanShift створена")
    ...
    # Збереження моделі
    save_cluster_model(model, "mean_shift_model", important_features)
    logger.info("Модель збережена у файл mean_shift_model.pkl")
    ...
    plt.xlabel("PCA 1")
    plt.ylabel("PCA 2")
    plt.title("Візуалізація кластерів (MeanShift, PCA)")
    ...
    plt.title("Середні значення ознак по кластерах (MeanShift)")
    ...
    plt.title("Розподіл кількості об'єктів у кластерах (MeanShift)")
    ...
    logger.info("=== Кінець функції train (MeanShift) ===")
