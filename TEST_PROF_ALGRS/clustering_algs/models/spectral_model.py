...
def train(df, target: str = None, params: dict = None):
    """
    Тренування моделі SpectralClustering.
    Параметри приймаються через словник params.
    """

    logger.info("=== Запуск функції train (SpectralClustering) ===")
    ...
    logger.info("Модель SpectralClustering створена")
    ...
    # Збереження моделі
    save_cluster_model(model, "spectral_model", important_features)
    logger.info("Модель збережена у файл spectral_model.pkl")
    ...
    plt.xlabel("PCA 1")
    plt.ylabel("PCA 2")
    plt.title("Візуалізація кластерів (Spectral, PCA)")
    ...
    plt.title("Середні значення ознак по кластерах (Spectral)")
    ...
    plt.title("Розподіл кількості об'єктів у кластерах (Spectral)")
    ...
    logger.info("=== Кінець функції train (SpectralClustering) ===")
